...
C04 users are global identities. They do not carry a workspace, organization, membership, role, or permission. A valid session proves only the global user identity; later tenant routes must independently establish workspace membership, authorize the action, and scope every repository call.

...
C05 persists organizations as account containers and workspaces as operational tenant boundaries, but it adds no membership, role, permission, owner-user field, or user tenancy association. It exposes no public organization/workspace route. Client headers, queries, bodies, and cookies do not establish workspace context. C06 must resolve membership server-side before setting trusted workspace context or invoking a workspace-owned repository. RLS remains future defense in depth after application authorization.

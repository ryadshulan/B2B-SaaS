...
Organizations are account containers; workspaces are the operational tenant boundary in a one-to-many relationship. C05 tenancy and C06 access repositories bind to an explicitly supplied database or transaction executor. Organization bootstrap uses one transaction to commit organization, initial workspace, and authenticated owner membership together. Membership insertion failure rolls back all three writes; no nested transaction is used. C05 exposes lifecycle disabling and no hard-delete repository operation, and C06 disables memberships instead of deleting them.

C06 resolves workspace access with parameter-bound joins across active user, membership, workspace, and organization records. Owner-sensitive mutation locks the workspace row with `SELECT ... FOR UPDATE` before counting active owners, providing the serialization point that prevents concurrent operations from removing every owner.

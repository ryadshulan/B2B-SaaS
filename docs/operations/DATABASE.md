...
Database integration tests use the configured PostgreSQL instance but create only cryptographically unique `c02_migration_*`, `c02_transaction_*`, `c04_auth_*`, and `c04_cli_*` schemas. Cleanup validates those prefixes before dropping the disposable schemas. Tests never issue `DROP DATABASE`, reset shared schemas, or assume SQLite equivalence.

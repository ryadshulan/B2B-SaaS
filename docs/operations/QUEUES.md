...
The worker is independently runnable and deployable. C03 intentionally ships an empty production handler registry and enqueues no production jobs. The API is not Redis-dependent and its `/health` and PostgreSQL-backed `/ready` contracts are unchanged.
